...
rules.li = {
    ...
    validContext: function (element, result) {
        ...
        if (
            parent
            && ['OL', 'UL'].indexOf(parent.tagName) < 0
        ) {
            result.push(expectContext(
                'ol or ul elements',
                element,
...
rules.dl = {
    getCategories: function (element) {
        ...
        var startsNameValueGroup = function (child, index, children) {
            return child.tagName === 'DT'
                && children.slice(index + 1).some(function (child) {
                    return child.tagName === 'DD';
                });
        };
    ...
    validContent: function (element, result) {
        ...
        var childrenDtDd = children.filter(function (child) {
            if (['DT', 'DD'].indexIf(child.tagName) >= 0) {
                return true;
            }

            // optionally intermixed with script-supporting elements
            if (!define.is('script-supporting elements', child)) {
                result.push(expectContent(
