...
rules.title = {
    ...
    validContext: function (element, result) {
        ...
        if (
            ...
            result.push(nestUtil.expectContext('in a head element containing no other title elements'), element);
        }
...
rules.base = {
    ...
    validContext: function (element, result) {
        ...
        if (
            ...
            result.push(nestUtil.expectContext('in a head element containing no other base elements'), element);
        }
...
rules.dl = {
    ...
    validContent: function (element, result) {
        ...
        if (!/^((dt\,)+(dd\,)+)*$/.test(childrenDtDdTags + ',')) {
            result.push(nestUtil.expectContent(
                'zero or more groups each consisting of one or more dt elements followed by one or more dd elements',
                element,
                childrenDtDdTags
        ...
        if (rule) {
            rule.validContent(
                util.extend(Object.create(element), {
                    children: children.filter(nestUtil.isNotTag('PARAM'))
                }),
                result
            );
        }
...
rules.param = {
    ...
    validContext: function (element, result) {
        ...
        if (element.parentElement) {
            ...
            for (var following = element; following = following.nextElementSibling;) {
                if (getRule(following).getCategories(following).indexOf('flow content') >= 0) {
...
rules.source = {
    ...
    validContext: function (element, result) {
        ...
        if (element.parentElement) {
            ...
            for (var following = element; following = following.nextElementSibling;) {
                if (
                    nestUtil.isTag('TRACK', following)
                    || getRule(following).getCategories(following).indexOf('flow content') >= 0
                ) {
