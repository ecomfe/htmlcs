    ...
    lint: function (enable, document, reporter) {
        ...
        var isInTail = function (element) {
            ...
            if (
                ...
                html.children.indexOf(element) < 0 &&
                body.children.indexOf(element) < 0
            ) {
                return false;
            }
        ...
        document.querySelectorAll('script:not([type]), script[type="text/javascript"]').forEach(function (script) {
            if (!isInTail(script)) {
                reporter.warn(script, 'Javascript contents are recommended to be imported in the tail of <body>.');
            }
        });
    }
