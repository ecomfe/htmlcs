    ...
    lint: function (getCfg, parser, reporter) {
        ...
        parser.on('opentag', function (name, attrs) {
            ...
            if (blockLevelElements.indexOf(name) !== -1) {                
                ...
                if (blockLevelElements.indexOf(parentTag) !== -1 && indentSize <= parentIndentSize) {                    
                    reporter.warn(
                        this.startIndex,
                        '028',
                        'Properly indent ' + name + ' element when it is a child element of ' + parentTag + ' (of a block, list, or table element).'
                    );                                        
                }                 

                console.log('Indent of ' + name + ' is ' + indentSize + ' and its parent is ' + parentTag + ' and parent indent is ' + parentIndentSize);
