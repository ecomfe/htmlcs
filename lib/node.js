...
util.extend(Node.prototype, {
    ...
    // nonstandard (simplized)
    insertBefore: function(node, child){
        var parent = this;

        if (node && child && child.parentNode === parent) {
            var pos = parent.childNodes.indexOf(child);

            

            if (node.ownerDocument !== parent.ownerDocument) {
                var newDocument = parent.ownerDocument;
                Node.walk(node, function (node) {
                    node.ownerDocument = newDocument;
                });
            }
        }
    },
...
// insert a node to given place
Node.insert = function (parent, child, pos) {
    var childNodes = parent.childNodes;
    childNodes.splice(pos, 0, child);
    child.parentNode = parent;
    child.parentElement = (parent && util.isElement(parent)) ? parent : null;

    if (pos === 0) {
        parent.firstChild = child;
        child.previousSibling = null;
    } else {
        var prev = childNodes[pos - 1];
...
Node.remove = function (child) {
    ...
    var childNodes = parent.childNodes;
    var pos = childNodes.indexOf(child);

    childNodes.splice(pos, 1);
    child.parentNode = null;
    child.parentElement = null;

    parent.firstChild = childNodes[0] || null;
    parent.lastChild = childNodes[childNodes.length - 1] || null;
